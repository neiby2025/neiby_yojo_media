...
export default function CookieSettings({ onToggle }: CookieSettingsProps) {
  ...
  const saveSetting = (category: string, value: boolean) => {
    // 個別のCookie設定を保存
    document.cookie = `neiby_consent_${category}=${value}; path=/; max-age=${365 * 24 * 60 * 60}`;
    
    // 全体の同意状況も更新
    const hasAnyConsent = Object.values({...settings, [category]: value}).some(v => v);
  ...
  const acceptAll = () => {
    const allEnabled = { analytics: true, advertising: true, personalization: true };
  ...
  const rejectAll = () => {
    const allDisabled = { analytics: false, advertising: false, personalization: false };
